...
add_action( 'admin_menu', 'categories_settings_s2' );

/** Step 1. Function that contains the menu-building code */
function categories_settings_s2() {
    $submenu_text = '<span class="submenu-text-edit">Section 2 &nbsp;</span>';
    ...
    add_posts_page(
        'Categories Settings 2',                          // page_title
        $submenu_text . $submenu_dashicon,              // menu_title
        'edit_posts',                                   // capability
        'categories_settings_identifier_2',               // menu_slug
        'categories_options_s2'                           // function
    );
}
...
function categories_options_s2() {
    if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
    }
    $opt_name = 'section-2';
    $hidden_field_name = 'section_2_submit_hidden';
        ...
        <div id="section-2">
            <table class="form-table">
                ...
                <tr class="image-section">
                    <th><?php _e("Section 2", 'menu-test' ); ?></th>
                    <td class="image-post">
                        <img src="<?php echo get_template_directory_uri() ?>/images/section-2.png">
                        <p class="description">(Section 2 looks like this.)</p>
                    </td>
                </tr>
                <tr class="checkbox-post">
                    ...
                    <td>
                        ...
                        $section_2_values = get_option('section-2'); // Get an array with the saved values
                        ...
                        $matched_category = array_intersect($section_2_values['categories_names'], $current_categories);
                    ...
                    <td>
                        <select name="order-by-parameter[]">
                            <?php foreach ($order_by_parameters as $order_by_param) { ?>
                                ...
                                <option <?php if ($section_2_values['sort-by'] == $order_by_param) {echo selected(true);} ?> value="<?php echo $order_by_param?>"><?php echo $order_by_param ?></option>
                            <?php } ?>
                        </select>
                    </td>
                </tr>
                    ...
                    <td>
                        <select name="asc-or-desc">
                            <?php foreach ($asc_or_desc as $param) { ?>
                                <!-- Matches saved value from database with the existing options -->
                                <option <?php if ($section_2_values['asc-or-desc'] == $param) {echo selected(true);} ?> value="<?php echo $param?>"><?php echo $param ?></option>
                            <?php } ?>
                        </select>
                    </td>
                </tr>
                <tr class="additional-settings" id="posts-per-page-2">
                    <th><?php _e("Posts per page:", 'menu-test' ); ?></th>
                    <td>
                        <input type="number" name="posts-per-page" value="<?php echo $section_2_values['posts-per-page']; ?>" placeholder="Set a number:">
                    </td>
                </tr>
                </tbody>
            </table>
